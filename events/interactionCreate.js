...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (painel) {
            if (interaction.isButton() && (interaction.customId.startsWith('tab_') || interaction.customId.startsWith('panel_back_') || interaction.customId.startsWith('confirm_close_point_') || ['config_set', 'config_avisos', 'config_logs', 'toggle_maint', 'toggle_channel_logs', 'toggle_dm_logs', 'toggle_activity_logs', 'toggle_notice_dms', 'send_test_log', 'toggle_panel_private_mode', 'toggle_vortex_role_remove_mode', 'set_vortex_auto_pending', 'set_vortex_auto_approved', 'publish_fac_hierarchy_panel', 'refresh_fac_hierarchy_panel', 'toggle_selected_log_channel', 'toggle_mirror_message_channel', 'open_adjust_call_v2', 'adjust_call_activate', 'adjust_call_deactivate', 'visual_set_color', 'visual_set_banner', 'visual_clear_target', 'toggle_absence_end_message', 'test_notice', 'clear_point_user', 'clear_point_no_billing', 'correct_point_close', 'close_selected_point', 'delete_point_correction_channel', 'cancel_close_point', 'show_all_points', 'show_user_point_sheet', 'set_absence_role', 'change_absence_return', 'profile_test', 'profile_register', 'profile_delete_no_billing', 'profile_list_registered', 'profile_toggle_billing', 'toggle_point_monitor', 'toggle_offline_charge', 'run_point_automation'].includes(interaction.customId))) {
            ...
            if (interaction.isStringSelectMenu() && (interaction.customId === 'select_command_permission_target' || interaction.customId === 'select_fac_hierarchy_target' || interaction.customId === 'select_open_point_user' || interaction.customId === 'select_visual_target')) {
                return runInteractionHandler(interaction, `Painel select: ${interaction.customId}`, () => painel.handleSelectMenu(interaction));
            }
