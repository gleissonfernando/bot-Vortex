...
async function ensureIndexes(db: Db) {
  await Promise.all([
    ...
    db.collection('munitions').createIndex({ guild_id: 1, identificador: 1 }, { unique: true }),
    db.collection('munitions').createIndex({ guild_id: 1, ativo: 1, ordem: 1 }),
    ...
    db.collection('order_items').createIndex({ guild_id: 1, order_id: 1 }),
    db.collection('order_items').createIndex({ guild_id: 1, munition_id: 1, created_at: -1 }),
