...
siteUsersRouter.post('/', requireManager, async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ ok: false, error: 'Dados invalidos.' });
...
siteUsersRouter.post('/:discordId/suspend', requireManager, async (req, res) => {
  const discordId = String(req.params.discordId);
  const user = await updateSiteUser(discordId, { status: 'suspended' } as any);
...
siteUsersRouter.post('/:discordId/activate', requireManager, async (req, res) => {
  const discordId = String(req.params.discordId);
  const user = await updateSiteUser(discordId, { status: 'active' } as any);
...
siteUsersRouter.post('/:discordId/ban', requireRole('admin'), async (req, res) => {
  const discordId = String(req.params.discordId);
  const user = await updateSiteUser(discordId, { status: 'banned' } as any);
...
async function writeSiteUserAudit(req: any, action: string, targetDiscordId: string, metadata: Record<string, unknown> = {}) {
...
async function fetchDiscordRoleIds(discordId: string) {
