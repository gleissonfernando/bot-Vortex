...
export async function createBackup(reason = 'manual') {
  ...
  for (const item of collections) {
    if (SKIPPED_COLLECTIONS.has(item.name)) continue;
...
export async function restoreBackup(filename: string) {
  ...
  for (const [name, docs] of Object.entries(payload.collections)) {
    if (!Array.isArray(docs) || SKIPPED_COLLECTIONS.has(name)) continue;
    const target = db.collection(name);
    await target.deleteMany({});
    if (docs.length) await target.insertMany(docs as any[]);
  }
...
function resolveBackupFile(filename: string) {
  if (!/^[\w.-]+\.json$/.test(filename)) throw new Error('Nome de backup invalido');
  const root = backupRoot();
  const filePath = path.resolve(root, filename);
  if (!filePath.startsWith(root + path.sep)) throw new Error('Caminho de backup invalido');
  return filePath;
}
