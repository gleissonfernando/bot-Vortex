...
async function readProfiles() {
    const now = Date.now();
    if (cachedProfiles && cacheExpiresAt > now)
...
function hasRegistrationData(profile) {
    return Boolean(profile && typeof profile === 'object' && (profile.approvedAt
...
function addProfileRecord(records, seen, guildId, userId, profile) {
    const normalizedUserId = normalizeDiscordId(userId);
    if (!normalizedUserId || !hasRegistrationData(profile))
        return;
...
export async function getRegisteredProfileRecords(guildId) {
    ...
    for (const [topKey, topValue] of Object.entries(data)) {
        ...
        for (const [profileKey, profile] of Object.entries(topValue)) {
            if (!profile || typeof profile !== 'object')
                continue;
            const userId = normalizeDiscordId(profile.userId) || normalizeDiscordId(profileKey);
            addProfileRecord(records, seen, profile.guildId || topKey, userId, profile);
        }
    }
    if (!requestedGuildId)
        return records;
...
export async function getRegisteredProfileMemberInputs(guildId) {
    const requestedGuildId = normalizeDiscordId(guildId);
    return (await getRegisteredProfileRecords(guildId))
        ...
        if (!resolvedGuildId)
            return null;
        const displayName = profileText(profile.displayName, profile.nomeGame && profile.idGame ? `${profile.nomeGame} | ${profile.idGame}` : '', profile.nomeGame, profile.discordTag, record.userId);
        return {
            guildId: resolvedGuildId,
            discordUserId: record.userId,
            username: profileText(profile.discordTag, profile.username, displayName, record.userId),
            globalName: profileText(profile.globalName, profile.discordName) || null,
            displayName,
            avatarUrl: profileText(profile.avatarUrl, profile.profileImageUrl) || null,
            bannerUrl: profileText(profile.bannerUrl, profile.profileBannerUrl) || null,
            highestRoleId: null,
...
export async function buildRegisteredMemberFilter(guildId) {
    ...
    if (globalUserIds.length)
        clauses.push({ discord_user_id: { $in: [...new Set(globalUserIds)] } });
    return clauses.length === 1 ? clauses[0] : { $or: clauses };
}
export async function isRegisteredProfile(guildId, userId) {
    const normalizedGuildId = normalizeDiscordId(guildId);
    const normalizedUserId = normalizeDiscordId(userId);
    if (!normalizedUserId)
        return false;
    return (await getRegisteredProfileRefs(normalizedGuildId)).some((ref) => (ref.userId === normalizedUserId && (!normalizedGuildId || !ref.guildId || ref.guildId === normalizedGuildId)));
}
