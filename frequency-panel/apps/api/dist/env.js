...
dotenv.config();
function applyAlias(target, sources) {
    if (process.env[target]?.trim())
        return;
...
function requiredSecret(name, minLength, blockedValues = []) {
...
function optionalSecret(name, minLength, blockedValues = []) {
    const value = process.env[name]?.trim() || '';
    if (!value)
        return '';
    return validateSecretValue(value, minLength, blockedValues) ? value : '';
}
function validateSecretValue(value, minLength, blockedValues = []) {
