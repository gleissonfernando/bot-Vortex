...
const PLATFORMS = ['twitch', 'kick', 'youtube', 'tiktok', 'facebook', 'trovo', 'custom'];
...
const optionalDiscordId = z.preprocess((value) => value === '' ? null : value, z.string().min(1).max(32).optional().nullable());
...
function detectPlatform(input) {
    try {
        ...
        if (host.includes('tiktok.com'))
            return 'tiktok';
        if (host.includes('facebook.com') || host.includes('fb.gg'))
            return 'facebook';
        if (host.includes('trovo.live'))
            return 'trovo';
    }
    catch {
        return null;
...
function canonicalUrl(input, platform) {
    const raw = String(input || '').trim();
    if (raw.includes('://'))
        return raw;
    ...
    if (platform === 'youtube')
        return slug.startsWith('@') ? `https://youtube.com/${slug}` : `https://youtube.com/@${slug}`;
    if (platform === 'tiktok')
        return `https://www.tiktok.com/@${slug}`;
    if (platform === 'trovo')
        return `https://trovo.live/s/${slug}`;
...
function validatePlatformUrl(url, platform) {
    ...
    if (!detected)
        throw new Error('URL invalida ou plataforma nao reconhecida.');
    if (detected !== platform)
        throw new Error(`URL informada pertence a ${PLATFORM_LABELS[detected]}, nao a ${PLATFORM_LABELS[platform]}.`);
    return true;
}
...
async function resolveTwitchChannel(url) {
    const login = extractSlug(url, 'twitch').toLowerCase();
    const token = await getTwitchToken();
    const clientId = process.env.TWITCH_CLIENT_ID;
    if (!login || !token || !clientId)
        return { streamerName: login || 'Streamer', channelUrl: canonicalUrl(url, 'twitch') };
    const response = await fetch(`https://api.twitch.tv/helix/users?login=${encodeURIComponent(login)}`, {
        headers: { Authorization: `Bearer ${token}`, 'Client-Id': clientId }
    }).catch(() => null);
    if (!response?.ok)
        return { streamerName: login, channelUrl: canonicalUrl(url, 'twitch') };
    const data = await response.json();
    const user = Array.isArray(data.data) ? data.data[0] : null;
    if (!user)
        throw new Error('Canal Twitch nao encontrado.');
    return {
...
async function checkKickLive(item) {
...
async function checkPageHeuristic(item) {
    const fallback = emptyStatus(item.channel_url);
    ...
    const live = (platform === 'youtube' && (lower.includes('"islive":true') || lower.includes('"is_live":true') || lower.includes('watching now'))) ||
...
async function checkLiveStatus(item) {
    if (item.platform === 'twitch')
        return checkTwitchLive(item);
    if (item.platform === 'kick')
        return checkKickLive(item);
    return checkPageHeuristic(item);
}
function allowedMentions(roleId, guildId) {
    if (!roleId)
        return { parse: [] };
    if (roleId === guildId || roleId === '@everyone')
        return { parse: ['everyone'] };
    return { parse: [], roles: [roleId] };
}
function mentionText(roleId, guildId) {
    if (!roleId)
        return '';
    if (roleId === guildId || roleId === '@everyone')
        return '@everyone';
...
async function sendDiscordLiveNotification(item, status) {
    const token = env.discordBotToken || process.env.DISCORD_TOKEN || process.env.DISCORD_BOT_TOKEN;
    if (!token)
        return { ok: false, error: 'DISCORD_TOKEN/DISCORD_BOT_TOKEN ausente.' };
...
function shouldCheck(item, now = Date.now()) {
...
async function processLiveItem(item) {
    if (!item._id || !shouldCheck(item))
        return;
    ...
    await (await collection('live_notifications')).updateOne({ _id: item._id }, {
        $set: {
...
async function monitorLiveNotificationsOnce() {
    ...
    monitorRunning = true;
    try {
        const rows = await (await collection('live_notifications'))
            .find({})
...
async function fetchDiscordGuildOptions(guildId) {
    const token = env.discordBotToken || process.env.DISCORD_TOKEN || process.env.DISCORD_BOT_TOKEN;
    if (!token)
        return { channels: [], roles: [], error: 'DISCORD_TOKEN/DISCORD_BOT_TOKEN ausente.' };
    ...
    return {
        ...
        roles: [
            ...
            ...(Array.isArray(roles) ? roles : [])
                ...
                .map((role) => ({ id: String(role.id), name: String(role.name || 'cargo'), color: Number(role.color || 0), mentionable: Boolean(role.mentionable) }))
        ],
        error: null
    };
}
async function liveStats(guildId, rows) {
...
livesRouter.get('/', asyncRoute(async (req, res) => {
    ...
    try {
        const rows = await (await collection('live_alert_configs')).find({ guild_id: guildId }).sort({ created_at: -1 }).toArray();
...
livesRouter.post('/', requireManager, asyncRoute(async (req, res) => {
    ...
    if (!platform)
        return res.status(400).json({ ok: false, error: 'Nao foi possivel identificar a plataforma da URL.' });
    ...
    if (existing)
        return res.status(409).json({ ok: false, error: 'Este canal ja esta cadastrado.', live: normalizeLive(existing) });
...
livesRouter.put('/:id', requireManager, asyncRoute(async (req, res) => {
    ...
    const guildId = input.guildId || defaultGuildId(req);
    const lives = await collection('live_alert_configs');
    const update = { custom_message: null, updated_at: new Date() };
    if (input.url) {
        const platform = input.platform || detectPlatform(input.url);
        if (!platform)
            return res.status(400).json({ ok: false, error: 'Nao foi possivel identificar a plataforma da URL.' });
    ...
    if (input.discordChannelId !== undefined)
        update.alert_channel_id = input.discordChannelId;
    if (input.discordRoleId !== undefined)
        update.mention_role_id = input.discordRoleId || guildId;
    const row = await lives.findOneAndUpdate({ _id: id, guild_id: guildId }, { $set: update }, { returnDocument: 'after' });
    if (!row)
        return res.status(404).json({ ok: false, error: 'Canal nao encontrado.' });
...
livesRouter.post('/:id/test', requireManager, asyncRoute(async (req, res) => {
    ...
    if (!id)
        return res.status(400).json({ ok: false, error: 'ID invalido.' });
    const item = await (await collection('live_alert_configs')).findOne({ _id: id, guild_id: defaultGuildId(req) });
    if (!item)
        return res.status(404).json({ ok: false, error: 'Canal nao encontrado.' });
