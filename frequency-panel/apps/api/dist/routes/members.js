...
export async function memberAvatarHandler(req, res) {
    let member = null;
    try {
        const members = await collection('discord_members');
        member = serializeDoc(await members.findOne({ id: req.params.id }));
    }
    catch {
        return res.status(404).end();
    }
    if (!member || !(await isRegisteredProfile(member.guild_id, member.discord_user_id)))
        return res.status(404).end();
